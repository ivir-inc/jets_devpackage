...
public class JoinFederationAction extends Action {
    private ControlWebClient controlWebClient;
    private DevPackageMainPage gui;

    public JoinFederationAction(ControlWebClient controlWebClient, DevPackageMainPage gui){
    ...
    public void perform(Component component) {
        try {
            QueryResponse response = controlWebClient.mutateJoin().get();
            gui.setQueryText(response.toString());
            if (response.toString().contains("connected")) {
                gui.disableJoinFedBtn();
            }
